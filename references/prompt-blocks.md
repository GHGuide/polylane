...
For each eligible item, run exactly one real `"$POLYLANE_PROJECT_ROOT/bin/polylane-refine.sh" propose`
or `"$POLYLANE_PROJECT_ROOT/bin/polylane-refine.sh" decline` invocation. `propose-or-decline`
is a conceptual decision phrase, NOT a subcommand. Never silently leave an eligible item
