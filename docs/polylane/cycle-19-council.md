# Cycle 19 council — optional domain gate recovery

## Decision

Accept `m19.1` and `c55` as locally complete: the committed optional-domain builder tip
was merged only after its matching DONE marker, and the fresh merged fixture proves that
a generic project reports `not-requested` without evidence or Git mutation while a
requested profile still commits its bundle, grade, and PASS evidence. This correction
does not close `m18.3`.
...
- The existing graph is a navigation aid only; its stale helper topology cannot replace
  a current source trace or a hermetic fixture.
