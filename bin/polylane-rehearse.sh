...
rehearse() {
  ...
  (
    ...
    printf '%s\n' 'ULTIMATE-GOAL: prove the unattended supervised lifecycle.' 'CURRENT-SUBGOAL: exercise the contract-v2 lifecycle.' "GOAL: CONTRACT-V2 nonce=$nonce lane=lane-a writes a/x." 'OWN: a/** and lane status evidence.' 'FORBIDDEN: b/**, main, and orchestration files.' 'PREDEFINED-SKILLS: fixture-test fixture-debug' 'LANE-SPECIFIC-SKILLS: fixture-review fixture-check' 'Read only the named kit once; do not enumerate or rediscover skills.' 'TEST-CADENCE: focused check before DONE.' 'DELEGATION: forbidden; no subagents or fan-out.' 'CHECK-CACHE: use polylane-check.sh with $PWD/.polylane/check-cache/ for expensive checks.' 'EXTERNAL-EVIDENCE: none.' 'Verify owned output before DONE.' 'Supervisor owns lifecycle.' "Write docs/status-lane-a.md with STATUS: lane-a DONE run=$nonce." > .polylane/lanes/lane-a.txt
    printf '%s\n' 'ULTIMATE-GOAL: prove the unattended supervised lifecycle.' 'CURRENT-SUBGOAL: exercise the contract-v2 lifecycle.' "GOAL: CONTRACT-V2 nonce=$nonce lane=lane-b writes b/y." 'OWN: b/** and lane status evidence.' 'FORBIDDEN: a/**, main, and orchestration files.' 'PREDEFINED-SKILLS: fixture-test fixture-debug' 'LANE-SPECIFIC-SKILLS: fixture-review fixture-check' 'Read only the named kit once; do not enumerate or rediscover skills.' 'TEST-CADENCE: focused check before DONE.' 'DELEGATION: forbidden; no subagents or fan-out.' 'CHECK-CACHE: use polylane-check.sh with $PWD/.polylane/check-cache/ for expensive checks.' 'EXTERNAL-EVIDENCE: none.' 'Verify owned output before DONE.' 'Supervisor owns lifecycle.' "Write docs/status-lane-b.md with STATUS: lane-b DONE run=$nonce." > .polylane/lanes/lane-b.txt
    printf '%s\n' 'ULTIMATE-GOAL: prove the unattended supervised lifecycle.' 'CURRENT-SUBGOAL: exercise the contract-v2 lifecycle.' "GOAL: CONTRACT-V2 nonce=$nonce integrator verifies GO or NO-GO." 'OWN: integrator branch and integration evidence.' 'FORBIDDEN: main and builder-owned paths before merge.' 'PREDEFINED-SKILLS: fixture-test fixture-debug' 'LANE-SPECIFIC-SKILLS: fixture-review fixture-check' 'Read only the named kit once; do not enumerate or rediscover skills.' 'TEST-CADENCE: focused acceptance then terminal acceptance.' 'DELEGATION: forbidden; no subagents or fan-out.' 'CHECK-CACHE: use polylane-check.sh with $PWD/.polylane/check-cache/ for expensive checks.' 'EXTERNAL-EVIDENCE: none.' 'Supervisor owns lifecycle.' "Write docs/status-integrator.md with STATUS: integrator DONE run=$nonce." "End docs/verify-integration.md with POLYLANE-VERDICT: GO run=$nonce or POLYLANE-VERDICT: NO-GO run=$nonce." > .polylane/lanes/integrator.txt
    printf '%s\n' seed > seed.txt
